...
function pagination(start) {
  ...
  jQuery.ajax({
    type: "POST",
    url: BASE_URL + "admin/contact_us/list/" + start + "/" + limit,
    data: {
      search: search,
    },
...
$("#edit_users").on("submit", function (e) {
  ...
  $.ajax({
    type: "POST",
    url: BASE_URL + "admin/contact_us/view",
    data: {
      id: contactUsItemId,
    },
    success: function (response) {
      console.log("ressou", response);
      if (response.err == 0) {
        var data = response.data;
...
function sendEmail(email, reply) {
  console.log("emailll", email);
  $.ajax({
    type: "POST",
    url: BASE_URL + "admin/contact_us/send_mail",
    data: {
      email: email,
      reply: reply,
    },
