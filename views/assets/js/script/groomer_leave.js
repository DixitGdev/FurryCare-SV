...
function pagination(start) {
  ...
  jQuery.ajax({
    type: "POST",
    url: BASE_URL + "admin/groomer_leave/list/" + start + "/" + limit,
    data: {
      search: search,
    },
    success: function (response) {
...
function html(list) {
  ...
  for (var i = 0; i <= list.length; i++) {
    if (i == list.length) {
      ...
      let sdate= new Date(list[i].start_date);
      let edate= new Date(list[i].end_date);
      let rdate = new Date(list[i].created_at);
      let options = {
        day: "2-digit",
        month: "2-digit",
        year: "numeric",
      ...
      let sformattedDate = sdate.toLocaleString("en-IN", options);
      let eformattedDate = edate.toLocaleString("en-IN", options);
      let rformattedDate = rdate.toLocaleString("en-IN", options);

      htmlData += "<tr><td>" + list[i].groomern + "</td>";
      htmlData += "<td >" + sformattedDate+ "</td>";
      htmlData += "<td >" + eformattedDate + "</td>";
      htmlData += "<td >" + list[i].reason + "</td>";
      htmlData += "<td >" + rformattedDate + "</td>";
      htmlData += "<td >" + list[i].status + "</td>";

      htmlData += "<td>";
      htmlData += `<a  onclick="view('${list[i]._id}', '${list[i].groomer}','${list[i].groomern}')"
...
function add_record() {
  ...
  $(".popup-title").html("<h2>Add Groomer Leave</h2>");
}
...
jQuery(document).ready(function ($) {
  ...
  $("#edit_users").on("submit", function (e) {
    ...
    if ($("#groomer").val() != "") {
      ...
      $.ajax({
        type: "POST",
        url: BASE_URL + "admin/groomer_leave/save",
        data: {
          id: $("#Id").val(),
          groomer: $("#groomer").val(),
...
function view(id, groomer_id, groomer_n) {
  ...
  setTimeout(() => {
    jQuery.ajax({
      type: "POST",
      url: BASE_URL + "admin/groomer_leave/view",
      data: {
        id: id,
      },
      success: function (response) {
        if (response.err == 0) {
          ...
          $("#date1").val(formatDate(data.start_date));
          $("#date2").val(formatDate(data.end_date));
          ...
          $(`input:radio[name=status][value=${data.status}]`).prop(
            "checked",
            true
          );
          $(".popup-title").html("<h2>Edit Groomer leave</h2>");
          $("#edit-modal").modal("show");
          setSelectedOption();
        }
      },
...
function remove(id) {
  ...
  Swal.fire({
    ...
    if (result.value) {
      $.ajax({
        type: "POST",
        url: BASE_URL + "admin/groomer_leave/delete",
        data: {
          id: id,
        },
        success: function (response) {
