...
function pagination(start) {
  ...
  jQuery.ajax({
    type: "POST",
    url: BASE_URL + "admin/groomer_products/list/" + start + "/" + limit,
    data: {
      search: search,
    },
...
function view(id) {
  setTimeout(() => {
    jQuery.ajax({
      type: "POST",
      url: BASE_URL + "admin/groomer_products/view",
      data: {
        id: id,
      },
      success: function (response) {
        if (response.err == 0) {
          ...
          $(".popup-title").html("<h2>Edit Groomer Produts</h2>");
...
function add_record() {
  Reset();
  $(".popup-title").html("<h2>Add Groomer Produts</h2>");
...
function remove(id) {
  ...
  Swal.fire({
    ...
    if (result.value) {
      $.ajax({
        type: "POST",
        url: BASE_URL + "admin/groomer_products/delete",
        data: {
          id: id,
        },
        success: function (response) {
...
jQuery(document).ready(function ($) {
  ...
  $("#edit_users").on("submit", function (e) {
    ...
    if ($("#Name").val() != "") {
      ...
      $.ajax({
        type: "POST",
        url: BASE_URL + "admin/groomer_products/save",
        data: {
          id: $("#Id").val(),
          name: $("#Name").val(),
          stock: $("#Stock").val(),
