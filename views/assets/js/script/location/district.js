...
function pagination(start) {
  jQuery.ajax({
    type: "GET",
    url:
      BASE_URL + "admin/district/list/" + start + "/" + limit + "/" + city_id,
    data: {},
...
function html(list) {
  ...
  for (var i = 0; i <= list.length; i++) {
    if (i == list.length) {
      ...
      htmlData += "<td>" + list[i].pin_code + "</td>";
      htmlData += "<td>";
      htmlData +=
...
function view(id) {
  jQuery.ajax({
    type: "POST",
    url: BASE_URL + "admin/district/view",
    data: {
      id: id,
    },
    success: function (response) {
...
function remove(id) {
  ...
  Swal.fire({
    ...
    if (result.value) {
      $.ajax({
        type: "POST",
        url: BASE_URL + "admin/district/delete",
        data: {
          id: id,
        },
        success: function (response) {
...
jQuery(document).ready(function ($) {
  ...
  $("#edit_district").on("submit", function (e) {
    e.preventDefault();
    ...
    if ($("#name").val() != "") {
      ...
      $.ajax({
        type: "POST",
        url: BASE_URL + "admin/district/save",
        data: {
          id: $("#Id").val(),
          name: $("#name").val(),
